...
import { CLEAR_SHIPPING_ERRORS, GET_SHIPPING_FAIL, GET_SHIPPING_REQUEST, GET_SHIPPING_SUCCESS, NEW_SHIPPING_FAIL, NEW_SHIPPING_REQUEST, NEW_SHIPPING_SUCCESS, UPDATE_SHIPPING_FAIL, UPDATE_SHIPPING_REQUEST, UPDATE_SHIPPING_SUCCESS } from "../Types/shippingTypes";
...
export const getAddressDetails = (id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`/api/shipping/${id}`);
...
export const createShipping = (addressData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post(`/api/address/add`, addressData, config);
...
// Clear All Errors
export const clearShippingErrors = () => (dispatch) => {
